...
class Agent:
    ...
    energy_min: float = -1  # Ideological range minimum
    ...
    def clip_energy(self, max_threshold: Optional[float] = None) -> None:
        ...
        """
        max_energy = self.energy_max if max_threshold is None else min(self.energy_max, max_threshold)
...
class UnionSim:
    ...
    def validate_parameter_balance(self) -> Dict[str, any]:
        ...
        total_broadcast_energy = (self.seed_size * targets_per_organizer * 
                                   self.broadcast_energy)
        
        # Check strategy balance (should be within factor of 3)
        ratio = total_broadcast_energy / total_1to1_energy if total_1to1_energy > 0 else float('inf')
        ...
        if ratio > 3.0:
            ...
            # Suggest adjustment
            suggested_broadcast = total_1to1_energy / (self.seed_size * targets_per_organizer)
        ...
        decay_ratio = estimated_total_decay / avg_outreach if avg_outreach > 0 else float('inf')
        
        if decay_ratio > 1.5:
            warnings.append(
                f"⚠️  DECAY OVERWHELMS OUTREACH: Decay is {decay_ratio:.1f}x larger than outreach energy"
            )
            suggested_delta = avg_outreach * 0.8 / n_agents / (1 - initial_engaged_fraction)
            recommendations.append(
            ...
            suggested_delta = avg_outreach * 0.5 / n_agents / (1 - initial_engaged_fraction)
            recommendations.append(
    ...
    def _initialize_agents(self) -> None:
        ...
        for node_id in node_ids:
            ...
            if node_id in potential_organizer_ids:
                ...
                energy_min = np.random.uniform(self.minimum_energy, self.mobilizing_threshold)
                energy_max = np.random.uniform(self.omega, 1.0)
            else:
                # Cannot become organizer: energy_max < omega
                energy_min = np.random.uniform(self.minimum_energy, self.mobilizing_threshold)
                energy_max = np.random.uniform(self.mobilizing_threshold, self.omega - 0.01)
            
            # Initial energy uniformly between -1 and mobilizing_threshold
            initial_energy = np.random.uniform(self.minimum_energy, self.mobilizing_threshold)
            # Clip to agent's ideological range
    ...
    def get_engaged_neighbor_mean_energy(self, agent_id: int) -> float:
        ...
        that buffers against demobilization. Passive neighbors don't help.
        """
        neighbors = list(self.graph.neighbors(agent_id))
        if not neighbors:
            return 0.0
        ...
        energies = [self.agents[n].energy for n in engaged_neighbors]
        return np.mean(energies)
    
    def update_energy(
        self, 
        agent_id: int, 
        delta_outreach: float = 0.0
    ) -> Dict[str, float]:
        """
        ...
        # Complex contagion: positive boost from high-energy neighborhood
        ...
        # Decay term: stronger when surrounded by passive neighbors
        # Only engaged (mobilized/organizer) neighbors buffer against decay
    ...
    def _one_to_one_organizing(
        ...
        """
        targets = [
            n for n in neighbors if self.agents[n].state != AgentState.ORGANIZER
        ]
        
        if not targets:
            return
        
        # Prefer untargeted agents, select randomly
        untargeted = [t for t in targets if not self.agents[t].is_targeted]
        
        if untargeted:
            # Choose randomly from untargeted agents
            target_id = random.choice(untargeted)
        else:
            # Continue targeting existing target or pick new one
            current_target = next(
            (t for t in targets if self.agents[t].targeting_organizer == organizer_id),
            None
            )
            target_id = current_target if current_target else random.choice(targets)
    ...
    def _broadcast_mobilizing(
        ...
        for target_id in targets:
            self._pending_energy_boosts[target_id] = (
                self._pending_energy_boosts.get(target_id, 0.0) + scaled_broadcast_energy
            )
    ...
    def step(self) -> Dict:
        ...
        for agent_id in self.agents:
            self.agents[agent_id].clip_energy(
                max_threshold=None if self.agents[agent_id].state == AgentState.ORGANIZER else self.omega
            )
