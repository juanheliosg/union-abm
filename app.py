...
from union_abm.engine import AgentState, UnionSim
...
def get_cached_layout(sim: UnionSim, force_recompute: bool = False) -> dict:
    ...
    try:
        ...
        if (
            not force_recompute
            and st.session_state.network_layout is not None
            and set(st.session_state.network_layout.keys()) == set(sim.graph.nodes())
        ):
            return st.session_state.network_layout

        # Compute new layout using Fruchterman-Reingold
        pos = nx.spring_layout(
            sim.graph, seed=42, k=2 / np.sqrt(len(sim.graph.nodes())), iterations=100
        )
...
def create_network_visualization(sim: UnionSim, color_by: str = "state") -> plt.Figure:
    ...
    # Prepare node sizes
    node_sizes = [
        100 + max(0, sim.agents[node].energy) * 300 for node in sim.graph.nodes()
    ]
    ...
    if color_by == "energy":
        ...
        nx.draw_networkx_edges(
            sim.graph, pos, edge_color="#cccccc", alpha=0.5, width=0.5, ax=ax
        )
        ...
        labels = {
            node: state_letters[sim.agents[node].state] for node in sim.graph.nodes()
        }
        ...
        node_colors = [
            state_colors[sim.agents[node].state] for node in sim.graph.nodes()
        ]
    ...
    nx.draw_networkx_labels(
        sim.graph, pos, labels=labels, font_size=6, font_color="black", ax=ax
    )
    ...
    ax.set_title(
        f"Tick {sim.tick} | Org: {n_org} | Mob: {n_mob} | Pass: {n_pass} (Colored {color_label})",
        fontsize=12,
    )
    ax.axis("off")
...
def create_network_from_snapshot(
    sim: UnionSim, snapshot: dict, color_by: str = "state"
) -> plt.Figure:
    """
    ...
    # Draw edges first
    nx.draw_networkx_edges(
        temp_graph, pos, edge_color="#cccccc", alpha=0.5, width=0.5, ax=ax
    )
    ...
    # Draw node labels
    nx.draw_networkx_labels(
        temp_graph, pos, labels=labels, font_size=6, font_color="black", ax=ax
    )
    ...
    color_label = "by Energy" if color_by == "energy" else "by State"
    ax.set_title(
        f"Tick {snapshot['tick']} | Org: {state_counts[2]} | Mob: {state_counts[1]} | Pass: {state_counts[0]} (Colored {color_label})",
        fontsize=12,
    )
    ax.axis("off")
...
def create_plotly_network_from_snapshot(sim: UnionSim, snapshot: dict) -> go.Figure:
    ...
    node_trace = go.Scatter(
        ...
        text=node_text,
        marker=dict(
            color=node_colors, size=node_sizes, line=dict(width=1, color="white")
        ),
    )
...
def create_metrics_charts(metrics_history: list) -> dict:
    ...
    if "energy_gradient" in df.columns:
        ...
        charts["energy_gradient"].update_layout(
            yaxis_title="ΔE (Energy Added/Subtracted)", xaxis_title="Tick"
        )
    ...
    fitness_fig = make_subplots(
        rows=2, cols=2, subplot_titles=("Reach", "DSI", "LWR", "Network Metrics")
    )
    ...
    # DSI
    fitness_fig.add_trace(
        go.Scatter(x=df["tick"], y=df["dsi"], mode="lines", name="DSI"), row=1, col=2
    )

    # LWR (handle inf values)
    lwr_values = df["lwr"].replace([np.inf, -np.inf], np.nan)
    fitness_fig.add_trace(
        go.Scatter(x=df["tick"], y=lwr_values, mode="lines", name="LWR"), row=2, col=1
    )
    ...
    fitness_fig.add_trace(
        go.Scatter(
            x=df["tick"], y=df["global_efficiency"], mode="lines", name="Efficiency"
        ),
        row=2,
        col=2,
    )
...
def create_experiment_comparison_charts(df: pd.DataFrame, group_by: str) -> dict:
    ...
    for val in unique_values:
        ...
        avg_data = (
            subset.groupby("tick")
            .agg({"total_energy": ["mean", "std"], "mean_energy": ["mean", "std"]})
            .reset_index()
        )
        avg_data.columns = [
            "tick",
        ...
        fig_energy.add_trace(
            go.Scatter(
                ...
                + list(
                    (avg_data["total_energy_mean"] - avg_data["total_energy_std"])[::-1]
                ),
                fill="toself",
    ...
    for val in unique_values:
        subset = df[df[group_by] == val]
        avg_data = (
            subset.groupby("tick")["organizer_count"].agg(["mean", "std"]).reset_index()
        )
    ...
    if "energy_from_outreach" in df.columns:
        fig_components = make_subplots(
            rows=1, cols=3, subplot_titles=("Outreach", "Contagion", "Decay")
        )

        for i, component in enumerate(
            ["energy_from_outreach", "energy_from_contagion", "energy_from_decay"]
        ):
            for val in unique_values:
                subset = df[df[group_by] == val]
        ...
        fig_components.update_layout(
            title=f'Energy Components by {group_by.replace("param_", "")}', height=400
        )
    ...
    for col_idx, metric in enumerate(["reach", "organizer_count", "total_energy"]):
        fig_box.add_trace(
            go.Box(
                x=final_ticks[group_by].astype(str), y=final_ticks[metric], name=metric
            ),
            row=1,
            col=col_idx + 1,
        )
...
def load_experiment_from_folder(
    folder_path: str,
) -> Tuple[Optional[pd.DataFrame], Optional[ExperimentConfig], Optional[str]]:
    """
...
def render_experiment_tab():
    ...
    with exp_tab0:
        ...
        if experiments_dir.exists():
            ...
            if exp_folders:
                ...
                exp_folders = sorted(
                    exp_folders, key=lambda x: x.stat().st_mtime, reverse=True
                )
                ...
                yaml_files = list(folder_path_obj.glob("*_config.yaml")) + list(
                    folder_path_obj.glob("*_config.yml")
                )
                ...
                if st.button("📂 Load Experiment", type="primary"):
                    with st.spinner("Loading experiment..."):
                        df, config, yaml_path = load_experiment_from_folder(
                        ...
                        if df is not None:
                            ...
                            st.session_state.loaded_experiment_path = (
                                selected_folder_path
                            )
                            ...
                            st.success(
                                f"✅ Loaded experiment from `{selected_folder_name}`"
                            )
                            ...
                            if config:
                                ...
                                st.info(
                                    f"🔬 {summary['n_parameter_combinations']} parameter combinations × {summary['n_trials_per_combination']} trials"
                                )

                            st.balloons()
                        else:
                            st.error(
                                "Failed to load experiment. Make sure the folder contains valid results CSV."
                            )
    ...
    with exp_tab1:
        ...
        with st.expander(
            "💡 Recommended Parameter Presets for Alpha Comparison", expanded=True
        ):
            st.markdown(
                """
        ...
        with col2:
            exp_description = st.text_area(
                "Description", height=68, help="Describe the purpose of this experiment"
            )
        ...
        **Recommended Formula:** `broadcast_energy ≈ outreach_energy / (broadcast_reach × total_agents)`
        ...
        with st.expander("🔧 Auto-Calculate Balanced Broadcast Energy"):
            ...
            with calc_col1:
                temp_outreach = st.number_input(
                    "Your 1-1 Energy", value=0.45, step=0.05, key="calc_outreach"
                )
            with calc_col2:
                temp_reach = st.number_input(
                    "Your Broadcast Reach", value=0.3, step=0.05, key="calc_reach"
                )
            with calc_col3:
                temp_total_agents = st.number_input(
                    "Total Agents", value=100, min_value=10, key="calc_agents"
                )

            suggested_broadcast = temp_outreach / (temp_reach * temp_total_agents)
            st.success(
                f"**Suggested Balanced Broadcast Energy: {suggested_broadcast:.4f}**"
            )
            st.caption(
                f"This ensures organizing (α=1) and mobilizing (α=0) have similar total energy input per tick."
            )
        ...
        with fixed_col1:
            n_departments = st.number_input(
                "Departments", min_value=2, max_value=10, value=5
            )
            agents_per_dept = st.number_input(
                "Agents/Dept", min_value=5, max_value=50, value=20
            )
            seed_size = st.number_input("Seed Size", min_value=1, max_value=10, value=3)

        with fixed_col2:
            outreach_energy = st.number_input(
                "Outreach Energy",
                min_value=0.0,
                max_value=1.0,
                value=0.45,
            ...
            broadcast_energy = st.number_input(
                "Broadcast Energy",
                min_value=0.0,
                max_value=0.2,
                value=0.01,
            ...
            broadcast_reach = st.number_input(
                "Broadcast Reach",
                min_value=0.01,
                max_value=1.0,
                value=0.3,
        ...
        with fixed_col3:
            mobilizing_threshold = st.number_input(
                "Mobilizing Threshold",
                min_value=0.0,
                max_value=1.0,
                value=0.6,
            ...
            organizer_potential = st.number_input(
                "Organizer Potential",
                min_value=0.0,
                max_value=1.0,
                value=0.3,
        ...
        if outreach_energy > 0 and broadcast_energy > 0:
            ...
            mobilizing_energy = (
                broadcast_energy * seed_size * broadcast_reach * total_agents
            )
            ratio = (
                organizing_energy / mobilizing_energy
                if mobilizing_energy > 0
                else float("inf")
            )
            ...
            if ratio < 0.33 or ratio > 3.0:
                st.warning(
                    f"⚠️ Energy imbalance detected! Ratio is {ratio:.2f}:1 (should be between 0.33-3.0). "
        ...
        with add_col2:
            ...
            mobilizing_threshold_fixed = (
                st.number_input(
                    ...
                    key="mobilizing_threshold_fixed",
                )
                if "mobilizing_threshold" not in selected_sweep_params
                else mobilizing_threshold
            )
            omega_fixed = (
                st.number_input(
                    "Omega (Organizer Threshold)",
                    min_value=0.5,
                    max_value=1.0,
                    value=0.7,
                    step=0.05,
        ...
        if "delta" not in selected_sweep_params and "alpha" in selected_sweep_params:
            ...
            if delta_fixed < decay_target_min or delta_fixed > decay_target_max:
                st.warning(
                    f"⚠️ **Decay Balance Warning**: For your alpha range, delta should be between "
        ...
        with exec_col2:
            n_ticks = st.number_input(
                "Ticks per simulation", min_value=10, max_value=1000, value=100
            )
        with exec_col3:
            base_seed = st.number_input(
                "Base seed", min_value=0, max_value=99999, value=42
            )
        ...
        if parameter_ranges:
            ...
            with sum_col3:
                estimated_time = (
                    summary["total_simulations"] * n_ticks * 0.01
                )  # rough estimate
    ...
    with exp_tab2:
        ...
        if st.session_state.experiment_config is None:
            ...
            st.info(
                f"Ready to run **{summary['total_simulations']}** simulations "
            ...
            if st.button("🚀 Run Experiment", type="primary"):
                ...
                with st.spinner("Running experiment..."):
                    ...
                    if use_parallel:
                        ...
                        df = runner.run(
                            n_workers=n_workers, progress_callback=update_progress
                        )
                    else:
        ...
        if st.session_state.experiment_results is None:
            ...
            if param_cols:
                ...
                group_by = st.selectbox(
                    ...
                    options=param_cols,
                    index=(
                        param_cols.index("param_alpha")
                        if "param_alpha" in param_cols
                        else 0
                    ),
                ...
                if charts:
                    # Display charts
                    st.plotly_chart(
                        charts.get("energy_comparison"), use_container_width=True
                    )
                    ...
                    with col1:
                        if "reach_comparison" in charts:
                            st.plotly_chart(
                                charts["reach_comparison"], use_container_width=True
                            )
                    with col2:
                        if "organizer_comparison" in charts:
                            st.plotly_chart(
                                charts["organizer_comparison"], use_container_width=True
                            )

                    if "components_comparison" in charts:
                        st.plotly_chart(
                            charts["components_comparison"], use_container_width=True
                        )

                    if "final_metrics_box" in charts:
                        st.plotly_chart(
                            charts["final_metrics_box"], use_container_width=True
                        )
                ...
                if len(param_cols) >= 2:
                    ...
                    final_df = (
                        df.groupby(["combo_id", "trial_id"] + param_cols)
                        .last()
                        .reset_index()
                    )
                    ...
                    pivot_data = (
                        final_df.groupby([hm_x, hm_y])[hm_metric].mean().unstack()
                    )
    ...
    with exp_tab4:
        ...
        if st.session_state.experiment_results is None:
            ...
            with export_col1:
                ...
                st.markdown(
                    "Export all results with parameters, seeds, and metrics by tick."
                )
                st.caption(
                    "Note: CSV and YAML are automatically saved when experiment completes."
                )
                ...
                if hasattr(st.session_state, "experiment_runner"):
                    summary_df = (
                        st.session_state.experiment_runner.get_summary_dataframe()
                    )
            ...
            with export_col2:
                ...
                uploaded_yaml = st.file_uploader(
                    "Upload YAML config", type=["yaml", "yml"]
                )
                if uploaded_yaml is not None:
                    try:
                        ...
                        st.success(
                            "Configuration loaded! Go to Configure tab to review."
                        )
                    except Exception as e:
            ...
            st.markdown(
                "Generate SVG snapshots of network evolution for a specific parameter combination."
            )
                ...
                with svg_col1:
                    combo_idx = st.selectbox(
                        ...
                        format_func=lambda i: str(
                            {
                                k: v
                                for k, v in param_grid[i].items()
                                if k in config.parameter_ranges
                            }
                        ),
                    )
                    trial_idx = st.number_input(
                        "Trial index",
                        min_value=0,
                        max_value=config.n_trials - 1,
                        value=0,
                    )

                with svg_col2:
                    steps_str = st.text_input(
                        "Steps to capture (comma-separated)", value="0, 25, 50, 75, 100"
                    )
                    try:
...
def main():
    ...
    with col1:
        if st.button("🔄 Initialize Simulation", type="primary"):
            ...
            st.session_state.network_layout = (
                None  # Clear cached layout for new simulation
            )
    ...
    if st.session_state.sim is not None:
        ...
        with col2:
            lwr_display = (
                f"{metrics['lwr']:.3f}" if metrics["lwr"] != float("inf") else "∞"
            )
        ...
        with tab2:
            ...
            if n_snapshots > 0:
                ...
                playback_color_by = (
                    "energy" if playback_color_mode == "Energy" else "state"
                )
                ...
                with ctrl_col2:
                    if st.button("⬅️ Previous"):
                        st.session_state.playback_tick = max(
                            0, st.session_state.playback_tick - 1
                        )

                with ctrl_col3:
                    if st.button("➡️ Next"):
                        st.session_state.playback_tick = min(
                            n_snapshots - 1, st.session_state.playback_tick + 1
                        )
                ...
                if auto_play:
                    ...
                    for i in range(st.session_state.playback_tick, n_snapshots):
                        ...
                        fig = create_network_from_snapshot(
                            sim, snapshot, color_by=playback_color_by
                        )
        ...
        with tab3:
            ...
            if st.session_state.metrics_history:
                ...
                if "energy_components" in charts:
                    st.plotly_chart(
                        charts["energy_components"], use_container_width=True
                    )
        ...
        with tab4:
            ...
            if st.session_state.metrics_history:
                ...
                with col1:
                    ...
                    st.write(
                        f"- Final organizers: {summary.get('final_organizer_count', 0)}"
                    )
        ...
        with tab5:
            ...
            if n_snapshots > 1:
                    ...
                    if st.button("🖼️ Generate SVG Archive"):
                        ...
                        with zipfile.ZipFile(
                            ...
                            for i, snapshot in enumerate(snapshots):
                                if i % svg_interval == 0:
                                    status_text.text(
                                        f"Generating SVG for tick {snapshot['tick']}..."
                                    )
                            ...
                            status_text.text(
                                f"✅ Generated {exported_count} SVG files!"
                            )
                ...
                st.info(
                    "Run the simulation for more ticks to export network evolution SVGs."
                )
        ...
        st.info(
            "👆 Configure parameters in the sidebar and click 'Initialize Simulation' to begin."
        )
